...
function Dashboard({ className, setId }) {
  ...
  const get = async (pageSize, status = 1, currentPage = 1) => {
    const url = `${config.api_host}/admin/v1/projects?status=${status}&row=${pageSize}&page=${currentPage}`;
    try {
  ...
  const getSearch = async (keyword) => {
    const url = `${config.api_host}/admin/v1/projects?keyword=${keyword}&status=1&row=10`;
          ...
          >
            <thead>
              <tr className="text-left">
                ...
                <th style={{ minWidth: "150px" }}>Nama Project</th>
