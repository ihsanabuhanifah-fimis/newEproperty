...
function Dashboard({ className, setId }) {
  ...
  const get = async (pageSize, status = 1, currentPage = 1) => {
    const url = `${config.api_host}/admin/v1/admins?status=${status}&row=${pageSize}&page=${currentPage}`;
    try {
  ...
  const getSearch = async (keyword) => {
    const url = `${config.api_host}/admin/v1/admins?keyword=${keyword}&status=1&row=10`;
          ...
          >
            <thead>
              <tr className="text-left">
                ...
                <th style={{ minWidth: "80px" }}>Updated Password</th>
                ...
                <tr key={user.id}>
                  ...
                  <td>
                    <button
                      onClick={() => {
                        setId(user.id);
                        history.push(`${match.url}/UpdatePassword`);
                      }}
                      className="btn btn-primary font-weight-bolder font-size-sm"
                    >
                      Updated Password
                  ...
                  <td>
                    <a
                      onClick={() => {
                        history.push(`${match.url}/${user.id}/child`);
                      }}
          ...
          <div className={`d-flex justify-content-between`}>
            <p>
              {pageSize > pagination.totalCount
                ...
                : pageSize}{" "}
              of {pagination.totalCount} Project
            </p>
            {/* begin::Header */}
